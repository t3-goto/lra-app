...
resource "aws_db_subnet_group" "rds_mysql_subnet_group" {
  name = "rds-mysql-${var.name}-${var.env}"
...
resource "aws_db_parameter_group" "rds_mysql_parameter_group" {
  name = "rds-mysql-${var.name}-${var.env}-parameter-group"
  family = "mysql5.7"
  description = "rds-mysql-${var.name}-${var.env}-parameter-group"
...
resource "aws_db_option_group" "rds_mysql_option_group" {
  name = "rds-mysql-${var.name}-${var.env}-option-group"
  option_group_description = "rds-mysql-${var.name}-${var.env}-option-group"
...
resource "aws_kms_key" "kms_key" {
  description = "rds-mysql-${var.name}-${var.env}-master-key"
...
resource "aws_kms_alias" "kms_alias" {
  name = "alias/rds-mysql-${var.name}-${var.env}"
...
resource "aws_db_instance" "rds_mysql_instance" {
  identifier = "rds-mysql-${var.name}-${var.env}"
  engine = "mysql"
